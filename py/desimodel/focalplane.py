...
def on_gfa(telra, teldec, ra, dec, buffer_arcsec=100):
    ...
    dec : array-like
        An array of Dec values for locations in the sky.
    buffer_arcsec : :class:`float`, optional
        A value in arcseconds on the sky of how much tolerance one would
        allow for seeing if a target is on the gfa.

    Returns
    -------
    tuple
        A list of targets with their respective indices in the
        RA and Dec list passed in that fall on certain GFAs denoted by the index
        in the accompanying list. A list equal in length with the accompanying
        list with the gfa location 0-9 as each element.

    Notes
    -----
    When using :func:`desimodel.footprint.find_points_in_tiles` with this
    function to check what points are on the GFAs, the default radius
    parameter should be set to 1.65 (degrees), so that boundary GFA area
    actually encompasses points normally outside of the tile.
    """
    # If any calculated area is under the threshold area, it is mathematically impossible
    THRESHOLD_AREA = 469.7
    MIN_TOLERANCE = 0.001

    inrangeindices = find_points_radec(telra, teldec, ra, dec, 1.65)
    if not inrangeindices:
        return np.array([]), np.array([])
    inrangeindices = np.asarray(inrangeindices)

    targetx, targety = radec2xy(telra, teldec, ra[inrangeindices], dec[inrangeindices])
...
def _retrieve_minimum_boundary(x_tolerance, y_tolerance):
    """A helper function to :func:`~desimodel.focalplane.on_gfa` to find the
    minimum boundary area for a point to lie inside a certain GFA given an
    tolerance in x and y in mm.

    Parameters
    ----------
    x_tolerance : :class:`float` or array-like
        Tolerance in x in mm.
    y_tolerance :
        Tolerance in y in mm.

    Returns
    -------
    array-like
        The minimum boundary area for the procedure to check if a point is
        inside the GFA.
    """
    ...
    #x, y = radec2xy(7.11, -10.53, targetx, targety)
    # If any calculated area is under the threshold area, it is mathematically impossible
    THRESHOLD_AREA = 469.7
    MIN_TOLERANCE = 0.001
    # The area boundary's value is the area of the gfa plus some tolerance.
...
def _degrees2xytolerance(buffer_arcsec):
    """A helper function to :func:`~desimodel.focalplane.on_gfa` to find
    the tolerance in x and y given a tolerance in arcseconds.

    Parameters
    ----------
    buffer_arcsec : :class:`float`
        A tolerance in arcseconds for checking if a point is on the GFA.

    Returns
    -------
    tuple
        The tolerances in x and y in mm.
    """
    # Uses the center of a given GFA from DESI-0530-v13 Excel Spreadsheet to find the tolerance
    from scipy.interpolate import interp1d
    platescale = load_platescale()
    fn = interp1d(platescale['radius'], platescale['radial_platescale'],
                  kind='quadratic')
    fn1 = interp1d(platescale['radius'], platescale['az_platescale'],
                   kind='quadratic')
...
def _shift_gfa_points(deltax, deltay):
    """A helper function to :func:`~desimodel.focalplane.on_gfa` to find
    the new GFA locations after incorporating a tolerance in x and y.
    ...
    -------
    tuple
        The 40 new GFA locations in x and y.
    """
...
def _find_new_gfa_coordinates(x, y, rotatemat):
    """A helper function to :func:`~desimodel.focalplane.on_gfa` to find
    the new GFA coordinates given a list of x coordinates, y coordinates,
    and a rotation matrix.

    Parameters
    ----------
    x : array-like
        A list of x coordinates for the GFAs.
    y : array-like
        A list of y coordinates for the GFAs.
    rotatemat : matrix-like
        A matrix for rotating the respective coordinates.

    Returns
    -------
    tuple
        A complete list of the 40 GFA x and y coordinates.
    """
...
def on_tile_gfa(tileid, targets, buffer_arcsec=100):
    ...
    buffer_arcsec : :class:`float`, optional
        Additional buffer region around GFA to include.

    Returns
    -------
    tuple
        A list of indices for targets that are covered by GFA number in
        corresponding the accompanying list of indices corresponding
        to 0-9 GFA location.
    """
